...
class TestSecretsUpdateNoteRequestModel(unittest.TestCase):
    ...
    def test_secrets_update_note_request(self):
        ...
        test_model = SecretsUpdateNoteRequest(
            note="minus", secret="unde", config="explicabo", project="necessitatibus"
        )
        self.assertEqual(test_model.note, "minus")
        self.assertEqual(test_model.secret, "unde")
        self.assertEqual(test_model.config, "explicabo")
