...
class TestUsers_(unittest.TestCase):
    ...
    def test_get(self):
        # Mock the API response
        responses.get(
            "https://api.doppler.com/v3/workplace/users/ex", json={}, status=200
        )
        # call the method to test
        test_service = Users("testkey")
    ...
    def test_get_required_fields_missing(self):
        # Mock the API response
        responses.get(
            "https://api.doppler.com/v3/workplace/users/fuga", json={}, status=202
        )
    ...
    def test_get_error_on_non_200(self):
        # Mock the API response
        responses.get(
            "https://api.doppler.com/v3/workplace/users/libero", json={}, status=404
        )
        with self.assertRaises(ClientException):
            test_service = Users("testkey")
            test_service.get("libero")
        responses.reset()
